function departmentAttendanceTab() {
    if (Ext.getCmp('department_attendance_tab')) {
        tab_panel.setActiveTab(Ext.getCmp("department_attendance_tab"));
    } else {
        var new_tab = tab_panel.add({
            ...
            closable: true,
            id: 'department_attendance_tab',
            autoScroll: true,
            items: [
                Ext.create('Ext.grid.Panel', {
                    id: 'department_attendance_list_grid',
                    //title: 'Attendance',
                    //height: 300,
                    //layout: 'fit',
                    autoScroll: true,
            ...
            tbar: [
                Ext.create('Ext.toolbar.Toolbar', {
                    border: false,
                    items: [
                        newCombo('', 'department', 'department_attendance_search_department_combo'),
                        newDateField('', 'form_date', 'department_attendance_search_form_date'), {
                            xtype: 'button',
                            ...
                            handler: function() {
                                var f = Ext.getCmp('department_attendance_search_form_date').value
                                var c = Ext.getCmp('department_attendance_search_department_combo').value
                                var d = new Date(f);
                                var v = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
                                if (f && c) {
                                    Ext.getCmp('department_attendance_list_grid').getStore().load({
                                        params: {
                                            form_date: v,
                                            department: c,
                                            tt: {
            ...
            bbar: [{
                ...
                handler: function() {
                    if (Ext.getCmp('department_attendance_list_grid')) {
                        Ext.getCmp('department_attendance_list_grid').getStore().load();
                    }
                }
            }, {
                ...
                handler: function() {
                    if (Ext.getCmp('department_attendance_list_grid')) {
                        var storeData = Ext.getCmp('department_attendance_list_grid').getStore().data.items;
                        var win = window;
                        ...
                        if (l > 0) {
                            ...
                            var htmlData = '<!DOCTYPE html>' +
                                ...
                                '<h2><u>' + storeData[0].data.department + '</u></h2>' +
                                '<h4>' +
                                p.getUTCDate() +
                            ...
                            for (var i = 0; i < storeData.length; i++) {
                                ...
                                htmlData += addLeadingZero(9, parseInt(storeData[i].data.employee));
                                htmlData += '</td>';
